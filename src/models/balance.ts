...
/**
 * 从 Tushare API 响应转换为 BalanceSheet 对象
 *
 * @param fields - 字段名数组 (snake_case)
 * @param items - 数据行数组
 * @returns BalanceSheet 对象数组
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export function fromApiResponse(fields: string[], items: any[][]): BalanceSheet[] {
  ...
  return items.map((item) => {
    ...
    return {
      ...
      totalAssets: parseNumber(data.total_assets),
      totalLiab: parseNumber(data.total_liab),
      totalHldrEqyExcMinInt: parseNumber(data.total_hldr_eqy_exc_min_int),
      totalHldrEqyIncMinInt: parseNumber(data.total_hldr_eqy_inc_min_int),
      totalCurAssets: parseNumber(data.total_cur_assets),
      totalCurLiab: parseNumber(data.total_cur_liab),
      totalNcaAssets: parseNumber(data.total_nca_assets),
      totalNcaLiab: parseNumber(data.total_nca_liab)
    }
  })
}
