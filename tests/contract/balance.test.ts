 ...
 * 资产负债表接口契约测试
 *
 * 验证 balancesheet 接口的请求和响应格式是否符合契约定义
 */
...
describe('资产负债表接口契约测试', () => {
  ...
  it('成功响应: 验证请求格式和响应数据结构', async () => {
    ...
    server.use(
      http.post(baseUrl, async ({ request }) => {
        ...
        return HttpResponse.json({
          ...
          msg: 'success',
          data: {
            fields: ['ts_code', 'end_date', 'ann_date', 'report_type', 'total_assets', 'total_liab', 'total_hldr_eqy_exc_min_int', 'total_hldr_eqy_inc_min_int', 'total_cur_assets', 'total_cur_liab', 'total_nca_assets', 'total_nca_liab'],
            items: [
              ['000001.SZ', '20231231', '20240328', '1', 1200000000000, 800000000000, 380000000000, 400000000000, 500000000000, 300000000000, 700000000000, 500000000000]
            ]
          }
        })
      })
    ...
    // 调用方法 - 应该成功返回资产负债表数据
    const result = await client.getBalanceSheet('000001.SZ')
  ...
  it('认证失败: token 无效返回 40001 错误', async () => {
    ...
    // 调用方法 - 应该抛出认证错误
    await expect(client.getBalanceSheet('000001.SZ')).rejects.toThrow()
  })
  ...
  it('空数据响应: 未披露数据返回空 items', async () => {
    ...
    // 调用方法 - 应该返回空数组
    const result = await client.getBalanceSheet('999999.SZ')
    expect(result).toBeInstanceOf(Array)
