 ...
 * 财务指标接口契约测试
 *
 * 验证 fina_indicator 接口的请求和响应格式是否符合契约定义
 */
...
describe('财务指标接口契约测试', () => {
  ...
  it('成功响应: 验证请求格式和响应数据结构', async () => {
    ...
    server.use(
      http.post(baseUrl, async ({ request }) => {
        ...
        // 验证请求格式
        expect(body.api_name).toBe('fina_indicator')
        ...
        return HttpResponse.json({
          ...
          msg: 'success',
          data: {
            fields: ['ts_code', 'end_date', 'ann_date', 'roe', 'roe_waa', 'grossprofit_margin', 'netprofit_margin', 'debt_to_assets', 'current_ratio', 'quick_ratio', 'eps', 'bps'],
            items: [
              ['000001.SZ', '20231231', '20240328', 15.2, 14.8, 35.5, 18.3, 65.2, 1.45, 1.12, 1.25, 8.50]
            ]
          }
        })
      })
    ...
    // 调用方法 - 应该成功返回财务指标数据
    const result = await client.getFinancialIndicators('000001.SZ')
  ...
  it('认证失败: token 无效返回 40001 错误', async () => {
    ...
    // 调用方法 - 应该抛出认证错误
    await expect(client.getFinancialIndicators('000001.SZ')).rejects.toThrow()
  })
  ...
  it('空数据响应: 未披露数据返回空 items', async () => {
    ...
    // 调用方法 - 应该返回空数组
    const result = await client.getFinancialIndicators('999999.SZ')
    expect(result).toBeInstanceOf(Array)
