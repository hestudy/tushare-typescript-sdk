...
describe('现金流量表接口契约测试', () => {
  ...
  it('成功响应: 验证请求格式和响应数据结构', async () => {
    ...
    // 调用方法 - 应该成功返回现金流量表数据
    const result = await client.getCashFlowStatement('000001.SZ')
    ...
    expect(result[0].nCashflowAct).toBe(15000000000)
  })
  ...
  it('认证失败: token 无效返回 40001 错误', async () => {
    ...
    // 调用方法 - 应该抛出认证错误
    await expect(client.getCashFlowStatement('000001.SZ')).rejects.toThrow()
  })
  ...
  it('空数据响应: 未披露数据返回空 items', async () => {
    ...
    // 调用方法 - 应该返回空数组
    const result = await client.getCashFlowStatement('999999.SZ')
    expect(result).toBeInstanceOf(Array)
